...
$$\mu_i = \text{intercept}_i + \beta_{\text{room},i} \cdot n_{\text{rooms},i} + \beta_{\text{dist},i} \cdot \mathrm{dist\_centre\_km}_i (+ \beta_{\text{under},i} + \beta_{\text{prop},i} + \beta_{\text{outdoor},i} + \text{premium}_i)$$
...
Here, $\beta_\text{under}$, $\beta_\text{prop}$ and the premium are only applied when the property is close to an Underground station (i.e. near_underground=1), it is a house (i.e. property_type=House) and it is leased by an agent (i.e. listing_type=Agent), respectively, while $\beta_\text{outdoor}$ is applied conditionally, depending on the type of its outdoor_space.
